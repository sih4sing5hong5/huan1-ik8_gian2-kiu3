...
# 				分數=句對應分數[閩南句]
# 			else:
			鉸過 = self.提一段出來(閩南句, 字數)
			分數 = 判斷.分數(鉸過)
			分數資料.append(分數)
			答案.append(1)
		print('閩南語訓練', len(答案))
		return (分數資料, 答案)
	def 試驗提一段出來(self, 題目檔):
		試 = []
		for 段 in self._讀語料.讀語料檔案(題目檔):
			鉸過 = self.提一段出來(段, 字數)
			試.append(鉸過)
		return 試
	def 試驗著毋著(self, 函式, 題目, 答案, 前幾常用=None):
		分數資料 = []
		for 鉸過 in 題目:
			分數 = 判斷.分數(鉸過)
			分數資料.append(分數)
...
#  	分類 = TGB.試驗(試驗函式, '國4', '閩4')
	for 字數 in [5, 10, 20, 50, 100, 200, 500]:
		...
		試華 = TGB.試驗提一段出來('../語料/TGB/分國閩/試.國語.gz')
		試閩 = TGB.試驗提一段出來('../語料/TGB/分國閩/試.閩南.gz')
