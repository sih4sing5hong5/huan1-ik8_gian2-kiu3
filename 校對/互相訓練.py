...
class 互相訓練:
	def 標字斷詞(self):
		...
		for 擺 in range(1, 6):
			...
			後一个華臺 = '../語料/華臺/{0:02}.訓.華臺一對一斷詞.txt.gz'.format(擺)
			辭典, 連詞 = 辭典連詞.產生而且加一个檔案(頂一个典藏)
			斷字詞.處理音標(辭典, 原本華臺, 後一个華臺)
			頂一个華臺 = 後一个華臺
			print('{0:02}擺標典藏字'.format(擺))
			後一个典藏 = '../語料/典藏/{0:02}.臺語文典藏典藏一對一.txt.gz'.format(擺)
			連詞資料 = []
			for 檔名 in ['../語料/訓.例句一對一斷詞.txt.gz', '../語料/附錄句一對一斷詞.txt.gz', 頂一个華臺]:
	...
	def 產生試驗(self):
		...
		斷字詞 = 華臺斷詞()
		for 擺 in range(1, 6):
			這馬典藏 = '../語料/典藏/{0:02}.臺語文典藏典藏一對一.txt.gz'.format(擺)
			print('{0:02}擺標華臺音'.format(擺))
			這馬華臺 = '../語料/華臺/{0:02}.試.華臺一對一斷詞.txt.gz'.format(擺)
